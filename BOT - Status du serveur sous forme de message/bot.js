...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("Nameless RP :", "https://cdn.discordapp.com/attachments/378590070338224128/757332615765426348/Logo_sans_ecriture.PNG")
    .setColor(0x722ea5)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Nameless RP' : '⚫ Nameless RP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nComment pouvez-vous rejoindre le serveur?','Vous pouvez rejoindre le serveur en entrant l\'ip __**nameless-rp.fr**__ dans **direct connect**. En bas se trouve l\'état du serveur pour voir combien de personnes sont en ligne et dans la file d\'attente.\n\u200b\n',false)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Actuele server status:',`${STATUS}\n\u200b\n`);
      embed.setColor(0x722ea5)
    }
    return embed;
  };
  ...
  const offline = function() {
    ...
    .setColor(0x722ea5)
    .addField('Server Status',':x: OFF',true)
    .addField('File d\'attente','?',true)
    .addField('Joueurs connectés','?\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Server Status',':white_check_mark: ON',true)
        .addField('File d\'attente',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Joueurs connectés',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
